# Complete Nginx Configuration Guide for AWS
...
The EC2 instance includes an **automated Nginx configuration manager** that eliminates manual configuration steps. Simply deploy containers with Docker labels, and Nginx is automatically configured.
...
**Result:** Immediately accessible at `http://EC2_IP/payments`
...
Access the EC2 instance using AWS Systems Manager Session Manager:
...
- `http://EC2_IP/metrics` → prometheus
...
**Step 4: Test from EC2 directly**
```bash
curl http://localhost/my-service
```

**Step 5: Check security group allows traffic**
...
```bash
# Check specific service (from EC2)
curl http://localhost/payments/health

# Check from outside
curl http://EC2_IP/payments/health
```
...
- `http://EC2_IP/auth/*`
- `http://EC2_IP/users/*`
- `http://EC2_IP/payments/*`
- `http://EC2_IP/orders/*`
- `http://EC2_IP/notifications/*`
      ...
      - name: Deploy to EC2
        run: |
          aws ssm send-command \
            ...
            --document-name "AWS-RunShellScript" \
            --parameters 'commands=[
              "docker pull registry.example.com/my-service:latest",
